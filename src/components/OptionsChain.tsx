...
export function OptionsChain({ s }: { s: DeskSnapshot }) {
  ...
  return (
    <Panel
      title={
        <span>
          Live Options Chain · <span className="text-accent">{asset}</span> <span className="num text-dim">{fmtNum(spot, cfg.decimals)}</span> · {expiry === 'ALL' ? 'All Expiries' : fmtExpiryLong(expiry)}
        </span>
      }
      right={
        <>
          ...
          {(['ALL', 'C', 'P'] as const).map((t) => (
            <Chip key={t} active={tf === t} onClick={() => setTf(t)}>{t}</Chip>
          ))}
          <Chip active={posOnly} onClick={() => setPosOnly((v) => !v)}>★ POS {posCount}</Chip>
        </>
      }
      bodyClass="p-0"
    >
      <div className="flex flex-wrap items-center gap-1 border-b border-line px-2.5 py-1">
        <Chip active={expiry === 'ALL'} onClick={() => setExpiry('ALL')}>ALL</Chip>
        {s.expiries.map((e) => (
            ...
            {rows.map((r) => {
              ...
              return (
                <tr key={r.symbol} className={`border-t border-line/60 [&>td]:px-1.5 [&>td]:py-[3px] hover:bg-white/[0.03] ${held ? 'bg-accent/[0.07]' : itm ? 'bg-white/[0.015]' : ''}`}>
                  ...
                  <td className="text-right font-semibold text-white">{fmtNum(r.strike, cfg.decimals > 1 ? 0 : 0)}</td>
                  ...
                  <td className="text-right text-text">{px(r.mark)}</td>
                  <td className="text-right text-warn">{(r.iv * 100).toFixed(1)}</td>
                  <td className={`text-right ${r.delta >= 0 ? 'text-up' : 'text-down'}`}>{r.delta.toFixed(2)}</td>
                  <td className="text-right text-dim">{r.gamma.toExponential(1)}</td>
                  <td className="text-right text-dim">{r.vega.toFixed(asset === 'SOL' ? 3 : 1)}</td>
                  <td className="text-right text-down">{r.theta.toFixed(asset === 'SOL' ? 3 : 1)}</td>
                  <td className="text-right text-dim">{fmtNum(r.oi, 0)}</td>
                  <td className="text-right text-dim">{fmtNum(r.volume, 0)}</td>
                  <td className={`text-right font-semibold ${r.position > 0 ? 'text-up' : r.position < 0 ? 'text-down' : 'text-muted'}`}>{held ? fmtNum(r.position, dp) : '·'}</td>
                  <td className={`text-right ${r.upnl >= 0 ? 'text-up' : 'text-down'}`}>{held ? fmtSignedUsd(r.upnl, 0) : ''}</td>
                </tr>
              );
            })}
            {!rows.length && (
              <tr>
                <td colSpan={15} className="py-8 text-center text-muted">No contracts match filter</td>
              </tr>
            )}
          </tbody>
        </table>
